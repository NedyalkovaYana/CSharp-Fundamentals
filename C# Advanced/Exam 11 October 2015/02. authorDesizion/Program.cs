        ...
        private static char[][] SpreadBunnies(char[][] matrix, int rows, int cols)
        {
            ...
            for (int row = 0; row <= rows; row++)
            {
                for (int col = 0; col <= cols; col++)
                {
                    if (matrix[row][col] == 'B')
                    {
                        if (row > 0) newLair[row - 1][col] = 'B'; //up
                        if (row < rows) newLair[row + 1][col] = 'B'; // down
                        if (col > 0) newLair[row][col - 1] = 'B'; // left
                        if (col < cols) newLair[row][col + 1] = 'B'; // right
                    }
                }
            }
