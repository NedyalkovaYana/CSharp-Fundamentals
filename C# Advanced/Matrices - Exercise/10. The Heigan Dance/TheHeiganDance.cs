namespace _10.The_Heigan_Dance
{
    ...
    class TheHeiganDance
    {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                //Check for spell linfluence from previous step
                ...
                if (IsCellReached(player.Position[0],
                    player.Position[1], hitRow, hitCol) &&
        ...
        static bool IsPlayerDamaged(Player player, int hitRow, int hitCol)
        {
            if (player.Position[0] > 0 &&
                !IsCellReached(player.Position[0] - 1,
                player.Position[1], hitRow, hitCol))
            {
                player.Position[0]--;
                return false;
            ...
            if (player.Position[1] > 0 &&
                !IsCellReached(player.Position[0],
                player.Position[1] - 1, hitRow, hitCol)) //Try move left
            {
                player.Position[1]--;
                return false;
            }
        ...
        static bool IsCellReached(int cellRow, int cellCol, int hitRow, int hitCol)
        {
            return (cellRow >= hitRow - 1) && (cellRow <= hitRow + 1)
                   && (cellCol >= hitCol - 1) && (cellCol <= hitCol + 1);
        }

        static bool IsGameOver(Player player, double bossHeiganBlood, string spell)
        {
